...
public class MineSweeper : IMineSweeper
{
    ...
    /// <exception cref="ArgumentNullException"></exception>
    /// <exception cref="ArgumentException"></exception>
    public void Restore(RestoreData restore)
    {
        if (restore == null)
            throw new ArgumentNullException(nameof(restore));

        RowCount = restore.RowCount;
        ColumnCount = restore.ColumnCount;
        BombCount = restore.BombCount;
        Status = restore.Status;

        var openedInnerCells = restore.OpenedCells.Select(x => new InnerCell(x.Index, x.IsBomb, x.NeighborBombCount) { IsOpen = true }).ToArray();
        var remainingCells = restore.ClosedCells.Select(x => new InnerCell(x.Index, x.IsBomb, x.NeighborBombCount)).ToArray();
        var cells = openedInnerCells.Union(remainingCells).OrderBy(x => x.Index).ToArray();

        if (cells.Length != RowCount * ColumnCount)
            throw new ArgumentException("cells.Count() != RowCount * ColumnCount");
